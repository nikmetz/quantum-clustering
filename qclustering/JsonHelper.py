...
def run_json_config(js, path=""):
    ...
    init_params = get_params(strategy=js.get("init_params", "random"), value=init_value, num_wires=wires, num_layers=layers, params_per_wire=params_per_wire)
    ...
    if circuit == "overlap":
        ...
        qvk = SwapQuantumVariationalKernel(wires, ansatz, init_params, device, shots, js.get("noise_model", None), logging_obj)
    else:
